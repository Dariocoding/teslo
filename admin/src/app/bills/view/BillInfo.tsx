...
const BillInfo: React.FC<IBillInfoProps> = (props) => {
  ...
  const onCancelBill = () => {
    Swal.fire({
      title: formatMessage({ id: "bills.delete.youSure" }),
      text: formatMessage({ id: "bills.delete.youWillNotBeAbleToRecover" }),
      icon: "warning",
      ...
      if (result.isConfirmed) {
        ...
        setBill({ ...req.data });
        Swal.fire(
          formatMessage({ id: "app.canceled" }),
          formatMessage({ id: "bills.delete.success" }),
          "success"
        );
      }
    });
      ...
      <div className="p-4">
        <div className="mb-6">
          <h6 className="font-semibold text-lg">
            <div className="flex items-center">
              <span className="text-xl">{formatMessage({ id: "bills.single" })}</span>{" "}
              <BadgeStatusOrder
                full={false}
                status={bill.status}
          ...
          <div className="flex lg:items-center items-start justify-start lg:justify-between lg:mt-1 lg:flex-row flex-col">
            ...
            <div className="text-xs">
              <strong>{formatMessage({ id: "bills.label.reference" })}:</strong>{" "}
          ...
          <div className="flex lg:items-center items-start justify-start lg:justify-between lg:mt-1 lg:flex-row flex-col">
            <div className="text-xs">
              <strong>{formatMessage({ id: "bills.label.dateCreated" })}:</strong>{" "}
              {dayjs(bill.dateCreated).format("DD/MM/YYYY HH:mm")}
            </div>
            <div className="text-xs">
              <strong>{formatMessage({ id: "bills.label.dateUpdated" })}:</strong>{" "}
        ...
        <div>
          ...
          <div className="flex items-start justify-start lg:gap-1 mt-1 flex-col">
            <div className="flex lg:items-center items-start justify-start lg:justify-between w-full lg:flex-row flex-col text-xs">
              <div>
                <strong>{formatMessage({ id: "providers.label.name" })}:</strong>{" "}
                {bill?.provider?.name}
              </div>
              <div>
                <strong>{formatMessage({ id: "providers.label.email" })}:</strong>{" "}
                {bill?.provider?.email}
              </div>
            </div>
            ...
            <div className="flex lg:items-center items-start justify-start lg:justify-between w-full lg:flex-row flex-col text-xs">
              <div>
                <strong>{formatMessage({ id: "providers.label.phone1" })}:</strong>{" "}
                {bill?.provider?.phone1}
              </div>
              <div>
                <strong>{formatMessage({ id: "providers.label.phone2" })}:</strong>{" "}
        ...
        <div className="mt-4">
          <div className="flex items-center justify-between gap-x-1">
            ...
            >
              {formatMessage({ id: "app.edit" })} <FaEdit className="ml-1" />
            </Link>
            <button
              type="button"
            ...
            >
              {formatMessage({ id: "app.delete" })}
              <TiCancel className="ml-1" />
            </button>
          </div>
          <button type="button" className="w-full btn btn-warning btn-xs lg:flex hidden">
            {formatMessage({ id: "app.exportPdf" })} <FaFilePdf className="ml-1" />
          </button>
        </div>
        ...
        <RenderIf isTrue={bill.description}>
          <div className="mt-4">
            <div>
              <h6 className="font-semibold text-lg">
                {formatMessage({ id: "bills.label.description" })}
              </h6>
