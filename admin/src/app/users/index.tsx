			...
			<div className="tile">
				<div className="mb-4">
					<Tabs setSelectedValue={setSelected} selectedValue={selectedValue}>
						...
						<Tab value={ValidRoles.SUPERVISOR}>{translate("users.supervisors")}</Tab>
						<Tab value={ValidRoles.SELLER}>{translate("users.sellers")}</Tab>
					</Tabs>
				</div>
				<TableUsers
					users={usersSelected}
					setUsers={(data: User[]) =>
						setData([
							...data,
							...users.filter(user => !user.roles.includes(selectedValue)),
						])
					}
