...
const DataTable: React.FunctionComponent<IDataTableProps> = props => {
	...
	}, [itemOffset, itemsPerPage, data, q, showAll]);
	...
	function search(rows: any[]) {
		...
		return rows.filter(row =>
			columns.some(
				column =>
					getIn(row, column)?.toString()?.toLowerCase()?.indexOf(q.toLowerCase().trim()) >
					-1
			)
		);
	}
	...
	return (
		<div>
			...
			<RenderIf isTrue={!loading && !md}>
				<div
					className={classNames(showResponsive ? "table-responsive" : "overflow-x-auto")}
				>
					<table className="table">
						<thead>
							<tr>
								{heading.map(h => (
									<Th key={h.field} heading={h} currentItems={currentItems} />
								))}
							</tr>
						</thead>
						<tbody>
