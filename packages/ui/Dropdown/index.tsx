...
const Dropdown: React.FunctionComponent<IDropdownProps> = (props) => {
  const { displayButton, inTable = false, classNameButton, placement, className } = props;
  ...
  return (
    <Menu
      as="div"
      className={classNames("inline-block text-left", !inTable && "relative", className)}
    >
      ...
      >
        <Menu.Items
          ...
          className={classNames(
            "absolute z-[200] mt-0 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none w-max overflow-hidden",
            !inTable && placement === "bottom" && "right-0 origin-top-right w-56",
            !inTable && placement === "right" && "left-14 origin-bottom-left top-0 w-44",
            !inTable && !placement && "right-0"
          )}
        >
          {props.children}
        </Menu.Items>
