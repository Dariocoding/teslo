      ...
      <table className="table">
        <thead>
          <tr>
            ...
            <th className="text-center text-dark">{translate("products.label.price")}</th>
        ...
        <tfoot className="text-sm">
          <tr>
            ...
            <td className="px-2 py-3 text-center">{formatter.format(order.subtotal || 0)}</td>
          </tr>
          <tr>
            <td colSpan={3} className="text-right font-bold pl-2 pr-4 py-3">
              I.V.A ({order.iva}%):
            </td>
            <td className="px-2 py-3 text-center">{formatter.format(parseFloat(IVA))}</td>
          </tr>
          <RenderIf isTrue={order.discount}>
            <tr>
              <td colSpan={3} className="text-right font-bold pl-2 pr-4 py-3">
                Discount
              </td>
              <td className="px-2 py-3 text-center">{order.discount} %</td>
            </tr>
          </RenderIf>
          <tr>
            <td colSpan={3} className="text-right font-bold pl-2 pr-4 py-3">
    ...
    <tr>
      <td>
        <div className="flex items-center">
          <span>
            <img src={imageProduct(item.product)} alt={""} width={40} className={"mr-2"} />
          </span>
          {item.title} {item.size ? <span className="text-xs ml-2">({item.size})</span> : null}
        </div>
      </td>
